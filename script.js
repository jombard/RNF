(function (rnfApp, jQuery) {
  ...
  function initTotalDistanceChart() {
    ...
    var getDistanceCovered = function () {
      return jQuery
        .ajax({
          type: "GET",
          url:
            "https://rememberingnottoforget.org.uk/wp-content/themes/rnf/RoundTheWorld.json",
    ...
    var buildChart = function (distanceCovered, distanceRemaining) {
      ...
      var distanceCoveredRem =
        distanceCovered - distanceCovered1 - distanceCovered1;
      var distanceData = [
        {
      ...
      var chartElement = document
        .getElementById("divTotalDistance")
        .getContext("2d");
    ...
    var buildContributorsTable = function (contributors) {
      ...
      for (var athlete in contributors) {
        ...
        var divKm = jQuery("<div />")
          .addClass("col-sm-4")
          .html(addKilometers(Math.round(contributors[athlete].total / 1000)));
        var divMi = jQuery("<div />")
          .addClass("col-sm-4")
          .html(addMiles(contributors[athlete].total / 1000));
        contributorsDiv.append(
          divRow.append(divAthlete).append(divKm).append(divMi)
        );
      }

      var viewContributorsLink = jQuery("<a />")
        .attr("href", "#")
        .html(
          "View Contributions <em class='glyphicon glyphicon-chevron-down'></em>"
        );
    ...
    var buildDistanceStatsTable = function (
      totalDistance,
      distanceCovered,
      distanceRemaining,
      lastUpdateTime
    ) {
      jQuery("#distanceTotal").html(
        addKilometers(totalDistance) + "<br>" + addMiles(totalDistance)
      );
      jQuery("#distanceCovered").html(
        addKilometers(distanceCovered) + "<br>" + addMiles(distanceCovered)
      );
      jQuery("#distanceRemaining").html(
        addKilometers(distanceRemaining) + "<br>" + addMiles(distanceRemaining)
      );
      jQuery("#lastActivity").html(
  ...
  function initGalleryCarousel() {
    ...
    var carousel = jQuery("<div />")
      .attr({ id: "gallery-carousel", "data-ride": "carousel" })
      .addClass("carousel");
    ...
    for (var i = 0; i < items.length; i++) {
      indicators.append(
        jQuery("<li />")
          .attr({ "data-target": "#gallery-carousel", "data-slide-to": i })
          .addClass(i === 0 ? "active" : "")
      );
    }
    ...
    var leftControl = jQuery("<a />")
      ...
      .attr({ href: "#gallery-carousel", role: "button", "data-slide": "prev" })
      .html(
        "<span class='glyphicon glyphicon-chevron-left' aria-hidden='true'></span>"
      );
    var rightControl = jQuery("<a />")
      .addClass("right carousel-control")
      .attr({ href: "#gallery-carousel", role: "button", "data-slide": "next" })
      .html(
